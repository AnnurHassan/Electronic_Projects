...
// tunable constants
const double Kp = 1.3; // if increased: turns will be taken faster, oscilates. if lower: turn slowly and might miss
...
int normalizeReading(int sensorReading)
{
...
void start()
{
  while (1)
  {
    sensor_values[0] = normalizeReading(analogRead(Sensor_1));
    sensor_values[1] = normalizeReading(analogRead(Sensor_2));
    sensor_values[2] = normalizeReading(analogRead(Sensor_3));
    sensor_values[3] = normalizeReading(analogRead(Sensor_4));
    sensor_values[4] = normalizeReading(analogRead(Sensor_5));
    sensor_values[5] = normalizeReading(analogRead(Sensor_6));

    error = calculateError(sensor_values);
    proportional = Kp * error;
    derivative = Kd * (error - previous_error);
    delta = BASE_SPEED + proportional + derivative;
